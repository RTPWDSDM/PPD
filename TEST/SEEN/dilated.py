...
if __name__ == '__main__':
    ...
    for k in range(1200): #dilate operation

        label_binary = cv2.imread(r"C:\Users\huang\Desktop\OPEN DATASET\DATASET\TEST\SEEN\PATH [2PIXEL]\%d.jpg" % k)

        kernel = np.uint8(np.ones((20,20))) #30，25, 20
        label_binary_dilate = cv2.dilate(label_binary,kernel)
        im = Image.fromarray(label_binary_dilate)
        im.save(r"C:\Users\huang\Desktop\OPEN DATASET\DATASET\TEST\SEEN\PATH [20PIXEL]\{}.jpg".format(k), dpi=(300, 300), quality=95)
